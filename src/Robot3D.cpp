...
// Note how everything depends on robot body dimensions so that can scale entire
// robot proportionately just by changing robot body scale
...
// Lighting/shading and material properties for robot - upcoming lecture - just
// copy for now Robot RGBA material properties (NOTE: we will learn about this
...
void initOpenGL(int w, int h) {
  ...
  glShadeModel(GL_SMOOTH); // Use smooth shading, makes boundaries between
                           // polygons harder to see
...
// Callback, called whenever GLUT determines that the window should be
// redisplayed or glutPostRedisplay() has been called.
void display(void) {
  glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

  glLoadIdentity(); // M = I
  // Create Viewing Matrix V
  // Set up the camera at position (0, 6, 22) looking at the origin, up along
  // positive y axis
...
void drawRobot() {
  ...
  // don't want to spin fixed base plate for this robot example so this is done
  // outside of robot push/pop above so it doesn't "inherit" the R_y(robotAngle)
  drawLowerBody();
...
void drawBody() {
  ...
  glScalef(robotBodyWidth, robotBodyLength,
           robotBodyDepth); // M = I V R_y(robotAngle) S
  glutSolidCube(1.0);
  glPopMatrix(); // M = I V R_y(robotAngle)
}

void drawHead() {
  // Set robot material properties per body part. Can have seperate material
  ...
  glTranslatef(0, 0.5 * robotBodyLength + 0.5 * headLength,
               0); // this will be done last
...
void drawLeftArm() {
  ...
  glTranslatef(0.5 * robotBodyWidth + 0.5 * upperArmWidth, 0,
               0.0); // this will be done last
...
void drawRightArm() {
  ...
  glTranslatef(-(0.5 * robotBodyWidth + 0.5 * upperArmWidth),
               0.5 * upperArmLength, 0.0);
...
void functionKeys(int key, int x, int y) {
  ...
  // else if (...)   // GLUT_KEY_DOWN, GLUT_KEY_UP, GLUT_KEY_RIGHT,
  // GLUT_KEY_LEFT
  //{
  //}
